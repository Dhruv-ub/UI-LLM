...
export default function CodeView({ user, accessToken, onAuthClick, onLogoutClick, onRefreshToken }) {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth('http://localhost:5000/conversations');
  ...
  const fetchMessages = useCallback(async (convId) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/messages/${convId}`);
      const data = await res.json();
      if (res.ok && data.status === 'success') {
      ...
      setMessages([
        {
          ...
          role: 'assistant',
          content: `Hello! Welcome to **AetherAI**. I am your professional AI assistant.\n\nThis dashboard is a fully interactive React user interface connected to a Node.js + Express backend.\n\n${
            user 
              ? `**You are logged in as \`${user.username}\`**! All conversations you start in the sidebar will be preserved in your database.` 
              : `**You are currently browsing anonymously**. Sign in using the button in the top header to save your conversations, view your sidebar history, and unlock persistent sessions.`
          }\n\nHow can I help you build today?`
        }
      ]);
    }
  }, [activeConvId, user, fetchMessages, isGenerating]);
  ...
  const handleDeleteChat = async (e, convId) => {
    ...
    try {
      const res = await fetchWithAuth(`http://localhost:5000/conversations/${convId}`, {
        method: 'DELETE'
      });
  ...
  const handleFileChange = async (e) => {
    ...
    if (ext === 'pdf') {
      ...
      try {
        ...
        const res = await fetchWithAuth('http://localhost:5000/upload', {
          method: 'POST',
          body: formData
        });
        const data = await res.json();
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await fetchWithAuth('http://localhost:5000/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    ...
    <div className="font-body-md text-body-md overflow-hidden h-[100dvh] flex w-full bg-background text-on-surface relative">
      ...
      <aside
        className={`fixed inset-y-0 left-0 md:relative h-[100dvh] bg-surface-container-low border-r border-outline-variant/10 shadow-2xl md:shadow-sm z-50 transition-all duration-300 flex flex-col ${
          isSidebarOpen 
        ...
        <div className="flex flex-col h-full p-4 md:p-md space-y-md overflow-hidden">
          ...
          <div className="flex items-center gap-xs justify-between w-full">
            <div className="flex items-center gap-xs">
              ...
              <div>
                <h1 className="font-headline-md text-[18px] md:text-headline-md font-bold text-primary">AetherAI</h1>
                <p className="font-label-sm text-[10px] md:text-label-sm text-on-surface-variant">Enterprise Pro</p>
              </div>
            </div>
            {/* Mobile close button */}
            <button 
          ...
          <div className="flex-grow overflow-y-auto scrollbar-hide space-y-lg mt-md">
            {user ? (
                  ...
                  {conversations.length === 0 ? (
                    ...
                    conversations.map((conv) => (
                      <div
                        ...
                        className={`rounded-lg p-2.5 md:p-sm flex items-center gap-xs cursor-pointer group transition-all ${
              ...
              <div className="flex flex-col items-center justify-center py-8 px-4 text-center gap-2 border border-outline-variant/10 rounded-xl bg-surface-container-low/50">
                <span className="material-symbols-outlined text-[36px] text-on-surface-variant/30">lock</span>
        ...
        <header className="flex justify-between items-center gap-2 md:gap-sm h-14 md:h-16 px-3 md:px-md w-full sticky top-0 z-30 bg-surface/80 backdrop-blur-xl border-b border-outline-variant/10">
          <div className="flex items-center gap-1 md:gap-sm min-w-0">
            ...
            <div className="flex items-center gap-1 md:gap-xs bg-surface-container-high rounded-full px-2 py-1 md:px-sm md:py-xs border border-outline-variant/20 cursor-pointer hover:bg-surface-container-highest transition-colors min-w-0">
              ...
              <span className="font-label-sm text-[12px] md:text-label-sm text-on-surface whitespace-nowrap hidden sm:inline-block">Aether-1 Pro</span>
          ...
          <div className="hidden lg:flex items-center gap-lg">
            ...
            <a className="text-on-surface-variant hover:text-on-surface font-label-sm text-label-sm transition-opacity" href="#">Benchmarks</a>
            <a className="text-on-surface-variant hover:text-on-surface font-label-sm text-label-sm transition-opacity" href="#">Documentation</a>
          </div>
          ...
          <div className="mx-auto flex w-full max-w-5xl flex-col items-center px-2 md:px-md pb-2 md:pb-3 pt-2 md:pt-4 lg:px-10 xl:px-16">
            ...
            >
              <textarea
                ...
                placeholder={user ? "Ask Aether anything..." : "Sign in to save chats..."}
                rows={1}
              />
